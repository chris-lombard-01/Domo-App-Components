...
The fix is `localStorage`: every selection (period, custom From/To, each dropdown, Compare) is saved to `navState` under the key `claudeNav.state.v1` as it's made. On load/reload, `selectPeriod(navState.period, false)` and `restoreSelectSelection(...)` read that back and re-push *that* filter instead of the static default — so a reload reproduces the same selection rather than resetting it.
...
## Why the bar kept refreshing/blinking — and the push guard that stops it

The reload-on-`filterContainer` behavior above cuts both ways. The restore step (`restoreSelectSelection` / `selectPeriod(navState.period, false)`) calls `domo.filterContainer()` on *every* load so a reload reproduces the right selection — but if pushing a filter is itself what triggers this app's reload, restoring unconditionally on every load re-pushes that same filter every single time the app reloads. Each push can retrigger another reload, which restores and pushes again, and so on: the page keeps refreshing and every number on it keeps blinking, forever.

The fix is a second, smaller piece of storage — a **push guard** (`pushGuard`, key `claudeNav.pushed.v1`, in `sessionStorage` rather than `localStorage`, since it only needs to survive this app's own reloads within one browser tab, not persist across visits). It remembers the last value actually pushed for each filter (`dropdown:Brand`, `dropdown:HFCMasterID`, ..., `period`). Restoring a value that's already marked as pushed updates this app's own UI (active button, filled pill) but **skips** the `domo.filterContainer()` call, since the page is presumably already showing it — breaking the loop after (at most) one push per filter per session. An actual user selection (a dropdown `change`, a period-button click, editing a Custom date) always pushes and re-marks regardless of the guard, via a `force`/`persist` flag threaded through `applyDateFilter()`/`handleSelectChange()` — only the passive restore-on-load path is guarded.

If the bar still won't stop refreshing after this: open the console. Nothing here logs on its own, but a quick `sessionStorage.getItem('claudeNav.pushed.v1')` will show whether the guard's recorded values are actually stabilizing (same value across reloads) or still changing every time (which would point to something else — e.g. `getRange()` computing a different "today" each call, such as right at a UTC/local-time day boundary).
