...
1. `domo.onFilterUpdate()` fires whenever any card on the page — the nav bar included — calls `domo.filterContainer()`. The handler keeps only `IN`-operator filters whose column is in `DIMENSION_COLUMNS` (`Brand`, `HFCMasterID`, `OwnerNumber`, `TerrNum` — the nav bar's exact `data-column` values) — an allowlist, not just "not `dt`" — so any date/period filter the nav bar pushes (its Reporting Period `BETWEEN` on `dt`, or anything else date-related under a different column name) is dropped rather than accidentally treated as a dimension filter. Kept filters are stored as `activeDimensionFilters`.
...
3. Every `onFilterUpdate` firing calls `renderAll()` again, which re-runs all four KPI queries with the updated `WHERE` clause — there's no cached row set to re-filter in memory the way the old version did, so a dimension change means a fresh (small, fast) round of aggregate queries rather than an instant in-memory re-filter.
